 ...
 */
export async function generateQuotesExcel(quotes: Quote[]): Promise<string> {
  ...
  quotes.forEach((quote) => {
    worksheet.addRow({
      ...
      partnerSmokingStatus:
        quote.partnerSmokingStatus === null
          ? ''
  ...
  const exportDir = path.resolve(config.files.exportDirectory);
...
export function getExportFilePath(fileName: string): string {
  return path.join(path.resolve(config.files.exportDirectory), fileName);
}
